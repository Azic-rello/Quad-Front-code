...
export default function HomeLayout() {
  ...
  return (
    <div className="w-full bg-gray-50 font-sans scroll-smooth">
      ...
      <div className="w-full bg-gradient-to-r from-orange-500 via-orange-600 to-yellow-500 shadow-xl relative overflow-hidden">
        ...
        <main className="max-w-7xl mx-auto px-4 sm:px-6 md:px-8 py-12 md:py-20 lg:py-28 z-10 relative">
          <section className="grid grid-cols-1 md:grid-cols-2 items-center gap-8 md:gap-12">
            ...
            {/* TEXT BLOKI */}
            <div className="w-full text-center md:text-left flex flex-col items-center md:items-start order-2 md:order-1" data-aos="fade-right">
              ...
              <p className="mt-4 md:mt-6 text-orange-50 leading-relaxed max-w-xl text-sm sm:text-base lg:text-lg">
                Best fast food delivered straight to your door. Fresh, hot and full of flavor.
              </p>
              <div className="mt-6 md:mt-8 flex flex-col sm:flex-row gap-3 sm:gap-4 w-full sm:w-auto justify-center md:justify-start">
                <button className="w-full sm:w-auto bg-yellow-400 hover:bg-yellow-500 text-gray-900 font-bold px-6 py-3.5 rounded-xl flex items-center justify-center gap-2 shadow-lg transition-all text-sm sm:text-base shrink-0 active:scale-95">
                  <span>View Menu</span>
                  <ArrowRight className="w-5 h-5" />
                </button>
                <button className="w-full sm:w-auto border border-white/60 hover:border-white hover:bg-white/10 text-white font-semibold px-6 py-3.5 rounded-xl transition-all text-sm sm:text-base shrink-0 active:scale-95">
                  About Us
                </button>
              </div>
            </div>
            ...
            {/* HERO RASM BLOKI */}
            <div className="w-full flex justify-center md:justify-end order-1 md:order-2" data-aos="fade-left">
              <div className="relative w-full max-w-[260px] sm:max-w-[340px] md:max-w-[380px] lg:max-w-[440px] aspect-square rounded-3xl overflow-hidden shadow-2xl md:rotate-3 transition-all duration-300 hover:rotate-0 hover:scale-105">
                <img src={burgerImg} alt="Special Burger" className="w-full h-full object-cover object-center select-none" />
              </div>
            </div>
      ...
      {/* Ortiqcha minus marginlar olib tashlandi, endi 100% pastki oq fon ichida turadi */}
      <div className="max-w-7xl mx-auto px-4 sm:px-6 md:px-8 pt-12 md:pt-16 relative z-20">
        <div className="grid grid-cols-1 sm:grid-cols-3 gap-4 md:gap-6">
          
          <div className="bg-white p-5 sm:p-6 rounded-2xl shadow-sm flex items-center gap-4 border border-gray-100 transition-all hover:-translate-y-1 hover:shadow-md" data-aos="zoom-in" data-aos-delay="100">
            <div className="p-3 bg-orange-100 text-orange-600 rounded-xl"><Truck className="w-6 h-6" /></div>
            <div>
              <h3 className="font-bold text-gray-800 text-sm sm:text-base">Free Delivery</h3>
              <p className="text-xs text-gray-500 mt-0.5">For orders over 100k UZS</p>
            </div>
          </div>

          <div className="bg-white p-5 sm:p-6 rounded-2xl shadow-sm flex items-center gap-4 border border-gray-100 transition-all hover:-translate-y-1 hover:shadow-md" data-aos="zoom-in" data-aos-delay="200">
            <div className="p-3 bg-red-100 text-red-600 rounded-xl"><Clock className="w-6 h-6" /></div>
            <div>
              <h3 className="font-bold text-gray-800 text-sm sm:text-base">30 Mins Delivery</h3>
              <p className="text-xs text-gray-500 mt-0.5">Average delivery time</p>
            </div>
          </div>

          <div className="bg-white p-5 sm:p-6 rounded-2xl shadow-sm flex items-center gap-4 border border-gray-100 transition-all hover:-translate-y-1 hover:shadow-md" data-aos="zoom-in" data-aos-delay="300">
            <div className="p-3 bg-yellow-100 text-yellow-600 rounded-xl"><Utensils className="w-6 h-6" /></div>
            <div>
              <h3 className="font-bold text-gray-800 text-sm sm:text-base">Hot & Fresh</h3>
              <p className="text-xs text-gray-500 mt-0.5">Made with whole wheat</p>
            </div>
          </div>
      ...
      <section className="max-w-7xl mx-auto px-4 sm:px-6 md:px-8 py-16 md:py-24">
        <div className="flex justify-between items-end mb-8 md:mb-12" data-aos="fade-up">
          <div>
            <span className="text-orange-600 font-bold text-xs sm:text-sm uppercase tracking-wider">Our Specials</span>
            <h2 className="text-2xl sm:text-3xl md:text-4xl font-black text-gray-900 mt-1">Menu</h2>
          </div>
          <button className="text-orange-600 font-bold text-sm hover:text-orange-700 flex items-center gap-1 transition-all">
            <span>View More</span>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
          {menuItems.map((item, idx) => (
            ...
            >
              {/* Haqiqiy rasmlar joylandi va to'liq moslashuvchan qilindi */}
              <div className="relative aspect-[4/3] bg-gray-50 overflow-hidden">
                ...
                />
                <span className="absolute top-3 right-3 bg-red-500 text-white font-bold text-xs px-2.5 py-1 rounded-full shadow-md">{item.price}</span>
              </div>
              <div className="p-5 flex-grow flex flex-col justify-between">
                <div>
                  <h3 className="font-bold text-gray-800 text-base md:text-lg mb-1">{item.name}</h3>
                  <p className="text-xs sm:text-sm text-gray-500 line-clamp-2 leading-relaxed mb-4">{item.desc}</p>
                </div>
