          ...
          <div className="space-y-3">
            ...
            <div className="flex items-center gap-2 p-3 bg-blue-50 border border-blue-200 rounded-xl">
              <span className="text-sm text-blue-800">
                Waiterga: <strong>{tipAmount.toLocaleString()} so'm</strong> (
                {tipPercent}%)
              </span>
            </div>
          </div>
...
const LiveOrder: React.FC = () => {
  ...
  const handleCloseBill = async (
    ...
    alert(
      `Table ${tableNumber} to'lovi qabul qilindi!\nIsh foizi: ${tipPercent}%`,
    );
        ...
        {isLoading && orders.length === 0 ? (
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            {[...Array(3)].map((_, i) => (
              <div
                key={i}
                className="bg-white rounded-3xl border border-stone-200/60 h-56 animate-pulse"
              />
            ))}
          </div>
        ) : orders.length === 0 ? (
