...
export default function Categories() {
  ...
  const fetchCategories = async (searchVal: string, pageVal: number) => {
    try {
      setLoading(true);
      const data = await categoryService.getAll(pageVal, limit, searchVal);
  ...
  // ✨ To'g'rilangan Debounce va useEffect effekti
  useEffect(() => {
    // Agar foydalanuvchi yozayotgan bo'lsa, yuklash stateni true qilib turamiz
    setLoading(true);

    const delayDebounceFn = setTimeout(() => {
      fetchCategories(search, page);
    }, 500); // 500ms kutish (ideal vaqt)

    return () => clearTimeout(delayDebounceFn);
  }, [page, search]); // Shu ikkalasi o'zgarganda ishlaydi
  ...
  const handleDelete = async (id: string) => {
    if (window.confirm("Rostdan ham ushbu kategoriyani o'chirmoqchimisiz?")) {
      try {
        ...
        fetchCategories(search, page); // Mavjud qidiruv va sahifani saqlab qolgan holda yangilash
      } catch (error) {
      ...
      <div className="relative mb-6 max-w-md">
        ...
        <input
          ...
          onChange={(e) => {
            ...
            setPage(1); // Qidiruv o'zgarganda darhol 1-sahifaga qaytadi
          }}
                ...
                {categories.map((category) => (
                  ...
                  >
                    ...
                    <td className="px-6 py-4">
                      <button
                        onClick={() =>
                          handleStatusChange(category.id, category.isActive)
                        }
        ...
        {!loading && categories.length > 0 && (
          <div className="flex items-center justify-between px-6 py-4 bg-gray-50 border-t border-gray-200 text-sm">
            <span className="text-gray-500">
              Sahifa <span className="font-semibold text-gray-800">{page}</span>{" "}
              / {totalPages}
            </span>
