...
export const ProductList: React.FC = () => {
  ...
  // ✅ MUHIM O'ZGARISH: Backenddan kelgan obyektdan massivni ajratib olamiz
  // Agar categoriesData.items mavjud bo'lsa, uni oladi, aks holda bo'sh massiv
    ...
    <div className="space-y-6">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          ...
          <p className="text-stone-500 text-sm mt-1">Mahsulotlarni qo'shish, tahrirlash va boshqarish</p>
        </div>
        {canManage && (
          <button onClick={handleCreate} className="px-5 py-2.5 bg-[#e31221] text-white rounded-lg hover:bg-red-700 transition-colors font-medium flex items-center gap-2 shadow-sm self-start sm:self-auto">
      ...
      <ProductFilters
        ...
        categories={safeCategories} // ✅ To'g'rilangan massiv yuboriladi
      ...
      {isLoading ? (
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">
          {[...Array(8)].map((_, i) => (<div key={i} className="bg-white rounded-xl shadow-sm h-80 animate-pulse border border-stone-100" />))}
        </div>
      ) : (
        <>
          {/* Grid */}
          {data?.items && data.items.length > 0 ? (
            ...
            <div className="text-center py-16 bg-white rounded-xl border border-dashed border-stone-300">
              <svg className="mx-auto h-12 w-12 text-stone-400 mb-3" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={1.5} d="M20 13V6a2 2 0 00-2-2H6a2 2 0 00-2 2v7m16 0v5a2 2 0 01-2 2H6a2 2 0 01-2-2v-5m16 0h-2.586a1 1 0 00-.707.293l-2.414 2.414a1 1 0 01-.707.293h-3.172a1 1 0 01-.707-.293l-2.414-2.414A1 1 0 006.586 13H4" /></svg>
              <h3 className="text-lg font-medium text-stone-900">Mahsulotlar topilmadi</h3>
              <p className="text-stone-500 mt-1">Qidiruv shartlarini o'zgartiring yoki yangi mahsulot qo'shing</p>
            </div>
          )}
          ...
          {data?.meta && data.meta.totalPages > 1 && (
            <div className="flex items-center justify-center gap-2 mt-8">
              <button onClick={() => setPage((p) => Math.max(1, p - 1))} disabled={page <= 1} className="px-4 py-2 border border-stone-300 rounded-lg hover:bg-stone-50 disabled:opacity-50 transition-colors text-sm font-medium">Oldingi</button>
              <span className="px-4 py-2 text-sm text-stone-600 font-medium bg-white border border-stone-200 rounded-lg">{data.meta.page} / {data.meta.totalPages}</span>
              <button onClick={() => setPage((p) => p + 1)} disabled={page >= data.meta.totalPages} className="px-4 py-2 border border-stone-300 rounded-lg hover:bg-stone-50 disabled:opacity-50 transition-colors text-sm font-medium">Keyingi</button>
            </div>
          )}
        </>
      )}
      ...
      {isFormOpen && (
        <ProductForm
          ...
          categories={safeCategories} // ✅ Faqat massiv yuboriladi
